...
def handler(event, context):
    ...
    try:
        # --- (اصلاح کلیدی) ---
        # بررسی می‌کنیم که آیا درخواست برای فایل است یا خیر
        path = event.get('path', '')
        if path.endswith(".ico") or path.endswith(".png"):
            logger.info(f"Ignoring static file request for: {path}")
            return {'statusCode': 200, 'body': 'Ignoring static file.'}
        # --- (پایان اصلاح) ---
        ...
        # حالا با اطمینان می‌دانیم که body یک JSON از تلگرام است
        ...
        # اگر خطای دیگری رخ دهد (مثلاً body خالی باشد)
